...
<head>
    ...
    <title>{{ __('Invoice') }} {{ $invoice->invoice_id }}</title>
    <style>
    ...
    <div class="container">
        <table class="header-table">
            <tr>
                ...
                <td class="company-details">
                    <h1>{{ __('فاتورة') }}</h1>
                    <p>
                        <strong>{{ __('رقم الفاتورة:') }}</strong> {{ $invoice->invoice_id }}<br>
                        <strong>{{ __('التاريخ:') }}</strong> {{ \Carbon\Carbon::parse($invoice->created_at)->format('Y-m-d') }}
                    </p>
                </td>
            </tr>
        </table>
        ...
        <div class="section">
             <table class="info-table">
                <tr>
                    <td class="label">{{ __('فاتورة إلى:') }}</td>
                    <td>
                        <strong>{{ $tenant?->user?->first_name }} {{ $tenant?->user?->last_name }}</strong><br>
                        {{ $tenant?->address ?? '-' }}<br>
                        {{ $tenant?->user?->email ?? '-' }}
                    </td>
                    <td class="label">{{ __('بيانات الشركة:') }}</td>
                     <td>
        ...
        <div class="section">
            <table class="items-table">
                <thead>
                    <tr>
                        <th>{{ __('الوصف') }}</th>
                        <th style="text-align: left;">{{ __('المبلغ') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($invoice->items as $item)
                        <tr>
                            <td>
                                {{ $item->description }}<br>
                                <small>{{ __(ucfirst($item->invoice_type)) }}</small>
                            </td>
                            <td style="text-align: left;">${{ number_format($item->amount, 2) }}</td>
                        </tr>
                    @endforeach
                    <tr class="total-row">
                        <td style="text-align: left;"><strong>{{ __('الإجمالي') }}</strong></td>
                        <td style="text-align: left;">
                            <strong>${{ number_format($invoice->items->sum('amount'), 2) }}</strong>
                        </td>
                    </tr>
                </tbody>
            </table>
